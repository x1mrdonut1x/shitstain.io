...
export default class GameEngine {
  ...
  private nextTimestep: number = Date.now();
  ...
  decrementScore(formerPegOwner) {
    ...
    const formerPegOwnerScore =
      +document.body.querySelector(formerPegOwnerElement).children[0].innerHTML;
  ...
  updateScore = (peg, chip) => {
    ...
    if (chipOwner !== formerPegOwner) {
      ...
      if (formerPegOwner) {
        this.decrementScore(formerPegOwner);
      }
    }
  };
  ...
  startGame() {
    ...
    this.loop = setInterval(() => {
      ...
      const interpolation = (Date.now() + TIMESTEP - this.nextTimestep) / TIMESTEP;

      this.env === 'client' &&
        this.chips.forEach(chip => {
          chip.sprite.destination = chip.body.position;
          chip.sprite.begin = chip.sprite.position;
          const incrementX = chip.sprite.destination.x - chip.sprite.begin.x;
          const incrementY = chip.sprite.destination.y - chip.sprite.begin.y;
